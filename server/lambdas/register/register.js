const https = require('https');
import { cognitoClient } from "../common/aws-clients.js";
import awsSettings from "../../../common/aws-settings.json";

exports.fetchRedcap = async (event) => {
    const rcid = event.pathParameters.rcid;
    if (!rcid || rcid === "") {
        return errResponse(400, "Required parameter rcid is missing.")
    }

    const data = {
        'token': process.env.TOKEN,
        'content': 'record',
        'action': 'export',
        'format': 'json',
        'type': 'flat',
        'records': rcid,
        'fields': 'first_name, last_name, email, phone',
        'events': 'participant_info_arm_1',
        'returnContent': 'count',
        'rawOrLabel': 'raw',
        'rawOrLabelHeaders': 'raw',
        'exportCheckboxLabel': false,
        'exportSurveyFields': false,
        'exportDataAccessGroups': false,
        'returnFormat': 'json'
    };

    const urlEncodedData = new URLSearchParams(data).toString();
    const options = {
        hostname: 'redcap.med.usc.edu',
        path: '/api/',
        headers: {
            "Content-Type": "application/x-www-form-urlencoded",
            "Content-Length": Buffer.byteLength(urlEncodedData),
            "Accept": "*/*"
        },
        method: 'POST'
    };

    const reqPromise = new Promise((resolve, reject) => {
        const req = https.request(options, (res) => {
            res.setEncoding('utf8');
            let data = "";
            res.on('data', (chunk) => {
                data = data + chunk.toString();
            });
            res.on('end', () => {
                resolve(data);
            });
            res.on('aborted', () => {
                console.log('rc response aborted');
                reject('redcap response aborted')
            });
        });
        req.on('error', (e) => {
            console.error('Error calling redcap: ', e);
            reject('Error calling redcap');
        });
    
        req.end(urlEncodedData, 'utf8');
    });

    try {
        const results = JSON.parse(await reqPromise);
        if (results.length != 1) {
            throw new Error(`Expected one result for rcid ${rcid} but got ${results.length}`);
        }
        return JSON.stringify(results[0]);
    } catch (err) {
        return {
            statusCode: 500,
            headers: {
                'x-amzn-ErrorType': err.code,
            },
            body: `Call to REDCap failed with error ${err.code}: ${JSON.stringify(err.message)}`
        };
    }
}

exports.signUp = async (event) => {
    try {
        // validate inputs
        const props = JSON.parse(event.body);
        const password = props.password;
        const phone = props.phone;
        const name = props.name;
        const email = props.email;
        for (const i of [password, phone, name, email]) {
            if (!i || i.trim().length == 0) {
                return errResponse(400, "One or more required parameters are missing.");
            }
        };
        if (password.length < 12) {
            return errResponse(400, "Password must be at least 12 characters.")
        }
        if (phone.length != 12 || !phone.match(/\+1[\d]{10}/)) {
            return errResponse(400, "Phone number must be in the form +12135551212.");
        }
        // email pattern from https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/email#validation
        if (!email.match(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/)) {
                return errResponse(400, "You must enter a valid email address.");
        }

        // call cognito to register user
        const params = {
            ClientId: awsSettings.ClientId,
            Username: phone,
            Password: password,
            UserAttributes: [
                {
                    Name: "name",
                    Value: name
                },
                {
                    Name: "email",
                    Value: email
                },
                {
                    Name: "phone_number",
                    Value: phone
                }
            ]
        }

        const resp = await cognitoClient.signUp(params);

        return {
            statusCode: 200,
            body: JSON.stringify({status: "success", sub: resp.UserSub}),
            headers: {
                "Access-Control-Allow-Origin": "*"
            }
        };
    } catch (err) {
        console.error(err);
        return errResponse(500, err.message);
    }
}

function errResponse(code, msg) {
    return {
        statusCode: code,
        body: msg,
        headers: {
            "Access-Control-Allow-Origin": "*"
        }
    }
}